...
export const getOrderList = async () => {
  ...
  try {
    ...
    if (error.message.includes("401")) {
      ...
      ctfAlert.alert_error("Lỗi xác thực, vui lòng đăng nhập lại.");
    } else {
      console.error("Lỗi khi lấy dữ liệu:", error);
      ctfAlert.alert_error("Lỗi khi lấy dữ liệu.", error.message);
    }
    return null;
  }
};
...
export const getOrderById = async (orderId) => {
  ...
  if (!orderId) {
    ...
    ctfAlert.alert_error("ID đơn hàng không hợp lệ.");
    return null;
  }
  ...
  try {
    ...
    if (response && response.data) {
      ...
      ctfAlert.alert_info("Không có chi tiết đơn hàng.");
      return null;
    }
  } catch (error) {
    if (error.message.includes("401")) {
...
export const createOrder = async (orderData) => {
  if (!token) {
    console.log("Token không tồn tại. Vui lòng đăng nhập.");
    ctfAlert.alert_error("Vui lòng đăng nhập trước khi tạo đơn hàng.");
    return;
  }
  ...
  try {
    ...
    if (response.status === "success") {
      ...
      ctfAlert.alert_success_v2("Đơn hàng đã được tạo thành công.");
      return response.data;
    } else {
      console.error(
        "Lỗi từ API:",
    ...
    ctfAlert.alert_error("Không thể tạo đơn hàng.", error.message);
    return null;
  }
};
  ...
  continueToPayment.addEventListener("click", async function (event) {
    ...
    try {
      ...
      if (Array.isArray(cartItems) && cartItems.length > 0) {
        ...
        ctfAlert.alert_info("Giỏ hàng rỗng.");
      }
    } catch (error) {
      ctfAlert.alert_error("Có lỗi xảy ra khi xử lý giỏ hàng.", error.message);
    }
    ...
    if (createdOrder && paymentMethod === "1") {
      ...
      ctfAlert.alert_info("Đang tiến hành thanh toán cho đơn hàng...");
      ...
      ctfAlert.alert_success(
        "Đơn hàng đã được tạo thành công với hình thức thanh toán bằng tiền mặt."
      );
    } else {
      console.log("Không thể tiến hành thanh toán vì tạo đơn hàng thất bại.");
      ctfAlert.alert_error(
        "Không thể tiến hành thanh toán vì tạo đơn hàng thất bại."
      );
    }
  });
}
